...
class InvoiceGenerator(tk.Tk):
    ...
    def generate_invoice(self):
        ...
        # Create a copy of the invoice_list to retain the data
        invoice_list_copy = self.invoice_list.copy()

        doc = DocxTemplate("invoice_template.docx")
        name = self.first_name_entry.get() + " " + self.last_name_entry.get()
        phone = self.phone_entry.get()
        subtotal = sum(item[3] for item in invoice_list_copy)
        salestax = 0.1
        total = subtotal * (1 + salestax)

        doc.render({"name": name,
                    "phone": phone,
                    "invoice_list": invoice_list_copy,
                    "subtotal": subtotal,
